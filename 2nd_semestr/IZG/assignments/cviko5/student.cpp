...
void initControlPointsUp2(S_Vector **points, int offset_x, int offset_y) {
    *points = vecCreateEmpty(sizeof(Point2d));
    Point2d p;
    p.x = 0;   p.y = 0;    point2d_vecPushBack(*points, p);
...
void initControlPointsDown(S_Vector **points, int offset_x, int offset_y) {
   ...
   */
  initControlPointsUp2(points, offset_x, offset_y);
}
...
void bezierCubic(const Point2d *P0, const Point2d *P1, const Point2d *P2, const Point2d *P3, 
   ...
   */
    Point2d B;
    double posun = 1.0 / quality;
    for (double t = 0.0; t <= 1.0; t += posun){
        B.x = P0->x * pow(1-t,3) + P1->x * 3 * t * pow(1 - t, 2) + P2->x * 3 * pow(t, 2) * (1 - t) + P3->x * pow(t, 3);
        B.y = P0->y * pow(1-t,3) + P1->y * 3 * t * pow(1 - t, 2) + P2->y * 3 * pow(t, 2) * (1 - t) + P3->y * pow(t, 3);
        point2d_vecPushBack(trajectory_points, B);
    }
   ...
   // Kostra:
    ...
    const Point2d *P3;
         for(int i = 0; i < point2d_vecSize(control_points) - 3; i += 3) {
             P0 = point2d_vecGetPtr(control_points, i);
             P1 = point2d_vecGetPtr(control_points, i + 1);
             P2 = point2d_vecGetPtr(control_points, i + 2);
             P3 = point2d_vecGetPtr(control_points, i + 3);
